...
private object FourBar {
    ...
    private val pivotPointUpperLinkage =
        getTranslation3d(z = 193.775.mm, x = -(52.3).mm)
    val upperLinkage =
        getPose3d().rotateAround(pivotPointUpperLinkage, (-45).deg.toRoll())
}

private object Body {
    val rollerConveyor by periodic {
        getPose3d(rotation = ConveyorRoller.inputs.position.toPitch())
    }
}
...
val mechanismPoses by periodic {
    ...
    subsystemPoseArray
    subsystemPoseArray[6] = Body.rollerConveyor
    subsystemPoseArray
}
