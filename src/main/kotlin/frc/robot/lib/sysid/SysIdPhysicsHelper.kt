...
fun createSysIdLinearConfig(
    ...
    val timeoutSeconds = max(timeAtMaxSpeed * timeoutSafetyMargin, minTimeoutSeconds)
    ...
    if (isVertical) {
        ...
        gravityVoltage = (gravityForceNewtons / maxForceNewtons) * BATTERY_VOLTAGE
    }

    val fwdStepVolts = min(gravityVoltage + stepVoltageDelta, maxStepVoltage)
    val fwdTargetQuasistaticVolts = min(gravityVoltage + quasistaticVoltageDelta, BATTERY_VOLTAGE)
    val fwdRampRate = fwdTargetQuasistaticVolts / timeoutSeconds

    val bwdStepVolts = min(max(stepVoltageDelta - gravityVoltage, minReverseVoltage), maxStepVoltage)
    val bwdTargetQuasistaticVolts = min(max(quasistaticVoltageDelta - gravityVoltage, minReverseVoltage), BATTERY_VOLTAGE)
