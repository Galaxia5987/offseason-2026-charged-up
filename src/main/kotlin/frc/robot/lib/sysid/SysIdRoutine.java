...
// Modified to fit CommandV3 architecture
...
public class SysIdRoutine extends SysIdRoutineLog {
    ...
    public static class SysIdMechanism {
         ...
         */
        public final Mechanism subsystem;
         ...
         */
        public SysIdMechanism(Consumer<Voltage> drive, Consumer<SysIdRoutineLog> log, Mechanism subsystem) {
    ...
    public Command quasistatic(Direction direction) {
        ...
        return Command.race(m_mechanism.subsystem.run(coroutine -> {
            timer.restart();
            coroutine.await(m_mechanism.subsystem.runRepeatedly(
                    () -> {
        ...
        }).whenCanceled(onExit).named("fullSysId"), Command.waitFor(m_config.timeout).named("SysIdTimeout")).named("sysid-" + state + "-" + m_mechanism.name);
    }
    ...
    public Command dynamic(Direction direction) {
        ...
        return Command.race(m_mechanism.subsystem.run(coroutine -> {
            output[0] = m_config.stepVoltage.times(outputSign);
            coroutine.await(m_mechanism.subsystem.runRepeatedly(
                    () -> {
